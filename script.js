document.addEventListener("DOMContentLoaded", () => {
  ...
  if (navUserBtn) {
    navUserBtn.addEventListener("click", (e) => {
      ...
      if (document.getElementById("profile-avatar")) {
        document.getElementById("profile-avatar").textContent = currentUser
          .charAt(0)
          .toUpperCase();
      }
      const totalGroupsCount = Object.keys(groupMemoryDatabase).length;
      if (document.getElementById("profile-groups-joined")) {
        document.getElementById("profile-groups-joined").textContent =
          totalGroupsCount;
      }
      if (document.getElementById("profile-groups-created")) {
        document.getElementById("profile-groups-created").textContent =
          totalGroupsCount;
      }
    });
  }
  ...
  const verAgendaHomeBtn = document.getElementById("ver-agenda-home-btn");
  if (verAgendaHomeBtn) {
    verAgendaHomeBtn.addEventListener("click", (e) => {
      e.preventDefault();
      resetActiveNav(); 
  ...
  const globalTasksContainer = document.getElementById(
    "global-tasks-container",
  );

  const classroomGroupName = document.getElementById("classroom-group-name");
  const classroomGroupSubject = document.getElementById(
    "classroom-group-subject",
  );
  ...
  const classroomBannerSchedules = document.getElementById(
    "classroom-banner-schedules",
  );
  const classroomMemberCount = document.getElementById(
  ...
  const btnInviteMemberModal = document.getElementById(
    "btn-invite-member-modal",
  );
  const btnCloseInvite = document.getElementById("btn-close-invite");

  const classroomMainWidgets = document.getElementById(
    "classroom-main-widgets",
  );
  const classroomMembersSection = document.getElementById(
    "classroom-members-section",
  );
  const classroomMembersList = document.getElementById(
    "classroom-members-list",
  );
  const classroomInviteSection = document.getElementById(
    "classroom-invite-section",
  );
  ...
  if (createGroupForm) {
    createGroupForm.addEventListener("submit", (e) => {
      e.preventDefault();
      const groupName = createGroupForm
        .querySelectorAll("input")[0]
        .value.trim();
      const groupSubject = createGroupForm
        .querySelectorAll("input")[1]
        .value.trim();
      const privacyValue = createGroupForm.querySelector("select").value;
      const privacyText =
        privacyValue === "privado" ? "Apenas convidados" : "Aberto para todos";
  ...
  function rebuildGridsHTML() {
    ...
    if (totalGroupsCreated === 0) {
      if (document.getElementById("empty-groups-state"))
        document
          .getElementById("empty-groups-state")
          .classList.remove("hidden");
      if (document.getElementById("empty-my-groups-state"))
        document
          .getElementById("empty-my-groups-state")
          .classList.remove("hidden");
      renderHomeSchedules();
      return;
    }
    ...
    if (document.getElementById("empty-groups-state"))
      document.getElementById("empty-groups-state").classList.add("hidden");
    if (document.getElementById("empty-my-groups-state"))
      document.getElementById("empty-my-groups-state").classList.add("hidden");
  ...
  if (searchBar && suggestionsBox) {
    searchBar.addEventListener("input", (e) => {
      ...
      const matchedGroups = Object.values(groupMemoryDatabase).filter(
        (group) =>
  ...
  if (joinPrivateForm) {
    joinPrivateForm.addEventListener("submit", (e) => {
      ...
      const targetGroupKey = Object.keys(groupMemoryDatabase).find(
        (key) =>
          groupMemoryDatabase[key].code === codeValue &&
          groupMemoryDatabase[key].privacy.includes("convidados"),
      );
      ...
      if (targetGroupKey) {
        ...
        privateErrorMsg.textContent =
          "Código privado não encontrado ou inválido.";
      }
    });
  }
  ...
  if (btnDeleteGroup) {
    btnDeleteGroup.addEventListener("click", () => {
      if (
        confirm(
          `Tem certeza que deseja excluir o grupo "${groupMemoryDatabase[currentEditingGroupId].name}"?`,
        )
      ) {
  ...
  if (btnAddSchedule) {
    btnAddSchedule.addEventListener("click", () => {
      ...
      if (!day || !timeValue)
        return alert("Selecione um dia e um horário válidos.");
      ...
      if (currentList.includes(record))
        return alert("Esse horário já está vinculado ao grupo!");
  ...
  function renderHomeSchedules() {
    const homeSessionsContainer = document.querySelector(
      "#view-inicio .dashboard-section:last-of-type",
    );
    if (!homeSessionsContainer) return;

    const sectionHeader =
      homeSessionsContainer.querySelector(".section-header");
  ...
  function renderTasks() {
    ...
    list.forEach((task, idx) => {
      ...
      card
        .querySelector(".task-checkbox-trigger")
      ...
      card
        .querySelector(".mural-post-btn-edit")
      ...
      card
        .querySelector(".mural-post-btn-delete")
        .addEventListener("click", () => {
          list.splice(idx, 1);
          renderTasks();
          renderGlobalTasks();
  ...
  function renderCalendar(monthIndex) {
    ...
    const isCurrentMonth =
      todayObj.getMonth() === monthIndex && todayObj.getFullYear() === anoAtual;
  ...
  function openDayDetailsModal(day, month, year) {
    ...
    if (dayModalTitle)
      dayModalTitle.textContent = `Eventos do dia ${dataFormatada}`;
    ...
    todosOsEventosDoDia.forEach((evt) => {
      ...
      if (evt.tipo === "grupo") {
        ...
        eventItem.querySelector(".delete-btn").addEventListener("click", () => {
          const globalIndex = window.globalEventsDatabase.indexOf(
            evt.originalObj,
          );
          if (globalIndex > -1)
            window.globalEventsDatabase.splice(globalIndex, 1);
  ...
  if (notificationBellBtn && notificationsModal) {
